...
void TabWidget::setTabCanMove(int index, bool canMove)
{
    ...
    if(widget)
    {
        if(canMove)
        {
            ...
            if(m_tabClosable)
            {
                ...
                btn->setStyleSheet("QPushButton{border-image: url(://close_normal.png);}"
                                   "QPushButton:hover{border-image: url(://close_hover.png);}");
...
void TabWidget::startDrag(int index)
{
	if (index >= 0)
	{
		QWidget* widget = this->widget(index);
...
void TabWidget::setTabCloseBtn(int index, QWidget *widget)
{
    QPushButton* btn=new QPushButton();
    btn->setStyleSheet("QPushButton{border-image: url(://close_normal.png);}"
                       "QPushButton:hover{border-image: url(://close_hover.png);}");
...
void TabWidget::tabBarCustomContextMenuRequested(const QPoint &pos)
{
    QTabBar* tabBar=this->tabBar();
...
void TabWidget::onCloseLeftTabAct()
{
...
void TabWidget::onCloseRightTabAct()
{
    QTabBar* tabBar=this->tabBar();
    int index=this->currentIndex();
...
void TabWidget::onCloseOtherTabAct()
{
    QTabBar* tabBar=this->tabBar();
...
void TabWidget::onFixedTabAct()
{
    setTabCanMove(currentIndex(),false);
}

void TabWidget::onUnFixedTabAct()
{

...
void MultiSelectComboBox::stateChange(int _row)
{
    ...
    for (int i = 1; i < count; i++)
    {
        ...
        {
            if(is_checkall&&i==checkallrow+1) continue;
            bool ok;
            check_box->text().toInt(&ok);
            if(!ok)selected_data.append(QString("%1").arg(i-1)).append(";");
            else selected_data.append(check_box->text()).append(";");
        }
    }
...
void MultiSelectComboBox::setCheckAllitem(int _row)
{
    QWidget *widget = list_widget_->itemWidget(list_widget_->item(_row+1));
    QCheckBox *check_box = static_cast<QCheckBox*>(widget);
    int count = list_widget_->count();
    connect(check_box,&QCheckBox::stateChanged,this,[=](){
        if(check_box->isChecked()) {
            for (int i = 1; i < count; i++)
            {
                if(i==_row+1)continue;
                QWidget *widget = list_widget_->itemWidget(list_widget_->item(i));
                QCheckBox *check_boxx = static_cast<QCheckBox*>(widget);
                check_boxx->setCheckState(Qt::Checked);
            }
        }
            ...
            for (int i = 1; i < count; i++)
            {
                ...
                QWidget *widget = list_widget_->itemWidget(list_widget_->item(i));
                QCheckBox *check_boxx = static_cast<QCheckBox*>(widget);
                if(!check_boxx->isChecked()) return;
            }
            for (int i = 1; i < count; i++)
            {
                if(i==_row+1)continue;
                QWidget *widget = list_widget_->itemWidget(list_widget_->item(i));
                QCheckBox *check_boxx = static_cast<QCheckBox*>(widget);
                check_boxx->setCheckState(Qt::Unchecked);
            }
        }
    });
    ...
    for (int i = 1; i < count; i++)
    {
        ...
        QWidget *widget = list_widget_->itemWidget(list_widget_->item(i));
        QCheckBox *check_boxx = static_cast<QCheckBox*>(widget);
        connect(check_boxx,&QCheckBox::stateChanged,this,[=](){
            if(!check_boxx->isChecked())  {
                if(check_box->isChecked()) check_box->setCheckState(Qt::Unchecked);
            }
        });
    }
    is_checkall=true;

...
#define plant_flash_time 0xbc
#define plant_imitater_target 0x138
#define plant_enable_shine 0x144
#define plant_towards 0x13c
#define plant_invisible 0x18
#define plant_target_zombie 0x12c
#define plant_shoot_time 0x58

#define bullet_offset 0xc8
#define bullet_disappear 0x50
#define bullet_count_max 0xcc
#define bullet_single_size 0x94
...
#define call_put_zombie_in_row 0x0040ddc0
...
#define zombie_xcorordinate_image 0x8
#define zombie_ycorordinate_image 0xc
#define zombie_xcorordinate 0x2c
#define zombie_ycorordinate 0x30
#define zombie_neardead 0xba
#define zombie_armor_shining_time 0x58
#define zombie_armor_shaking_time 0x5c
#define zombie_visible 0x18
#define zombie_eat_garlic 0xbf
#define zombie_in_water 0xbd
#define zombie_no_hand 0xbb
#define zombie_blow_away 0xb9
#define zombie_maxmium 0x98
...
#define frame_duration 0x454
#define mouse_grab_type 0x30
#define imitater_type 0x2c
...
inline HWND FindWindowFromProcessId(DWORD processId)
{
    HWND hwnd = NULL;
    // 获取系统中所有窗口的句柄
    hwnd = FindWindow(nullptr, nullptr);
    // 遍历每一个窗口，查找与指定进程关联的窗口
    while (hwnd != NULL)
    {
        DWORD windowProcessId = 0;
        // 获取窗口所属进程的ID
        GetWindowThreadProcessId(hwnd, &windowProcessId);
        // 如果找到与指定进程ID匹配的窗口，返回窗口句柄
...
inline void _CodeInject(HANDLE pvzHand,CodeStruct* target)
{
    // 在另一个进程的虚拟地址空间中分配内存
    ...
    if (addr == nullptr) {
        if(out.device()!=nullptr){
            ...
            out << QString("[%1] Error: Failed to alloc virtual memory, error code: %2.\n").arg(currentTime).arg(GetLastError());
        }
        return;
    }
    ...
    if (ret == 0 || write_size != target->length)
    {
        if(out.device()!=nullptr){
            QString currentTime = QDateTime::currentDateTime().toString("yyyy-MM-dd hh:mm:ss.zzz");
            out << QString("[%1] Error: Failed to write memory, error code: %2.\n").arg(currentTime).arg(GetLastError());
        }
    ...
    if (thread == nullptr)
    {
        if(out.device()!=nullptr){
            QString currentTime = QDateTime::currentDateTime().toString("yyyy-MM-dd hh:mm:ss.zzz");
            out << QString("[%1] Error: Failed to create remote thread, error code: %2.\n").arg(currentTime).arg(GetLastError());
        }
        VirtualFreeEx(pvzHand, addr, 0, MEM_RELEASE);
        return;
    }
    //待远程线程的执行完成，并在完成后关闭线程句柄以及释放先前分配的内存空间。
    DWORD wait_status = WaitForSingleObject(thread, 2000);
    if (wait_status == WAIT_FAILED) {
        if(out.device()!=nullptr){
            QString currentTime = QDateTime::currentDateTime().toString("yyyy-MM-dd hh:mm:ss.zzz");
            out << QString("[%1] Error: WaitForSingleObject failed with error, error code: %2.\n").arg(currentTime).arg(GetLastError());
        }
    }
    else if(wait_status==WAIT_TIMEOUT){
        if(out.device()!=nullptr){
            QString currentTime = QDateTime::currentDateTime().toString("yyyy-MM-dd hh:mm:ss.zzz");
            out << QString("[%1] Error: WaitForSingleObject timed out, please check if executed successfully.\n").arg(currentTime);
        }
    }
    CloseHandle(thread);
    VirtualFreeEx(pvzHand, addr, 0, MEM_RELEASE);
    ...
    for (auto it = addr.begin(); it != addr.end(); it++)
    {
        if (it != addr.end() - 1)
        {
            ...
            if (ret == 0 || sizeof(offset) != read_size){
                if(out.device()!=nullptr){
                    QString currentTime = QDateTime::currentDateTime().toString("yyyy-MM-dd hh:mm:ss.zzz");
                    out << QString("[%1] Error: Failed to read process memory, error code: %2.\n").arg(currentTime).arg(GetLastError());
                }
                return T();
            }
        }
    ...
    for (auto it = addr.begin(); it != addr.end(); it++)
    {
        ...
        else
        {
            ...
            if (ret == 0 || sizeof(value) != write_size)
            {
                if(out.device()!=nullptr){
                    QString currentTime = QDateTime::currentDateTime().toString("yyyy-MM-dd hh:mm:ss.zzz");
                    out << QString("[%1] Error: Failed to write process memory, error code: %2.\n").arg(currentTime).arg(GetLastError());
                }
                return;
            }
        }
